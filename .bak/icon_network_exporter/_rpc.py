...
class iconRPC:
    ...
    def __init__(self, config: Config):
        self.config = config
        self.rpc_url = self.config.discovery_node_rpc_url
        self._next_id = 1
        self._counter_discovery_node = Counter('icon_counter_discovery_node', 'Total number of calls to edge')
        self._counter_discovery_node_error = Counter('icon_counter_discovery_node_error',
                                                     'Total number of call errors to edge')
        self._counter_rpc_node_reply_status = Counter('icon_counter_node_reply_status', 'Total number of OK reply',
                                                      ["node_ID", "name", "status"])


    def request_nothrow_discovery_node(self):
        self._next_id += 1
        self._counter_discovery_node.inc()
        try:
            result = requests.post(self.rpc_url, data=json.dumps(GET_PREPS_RPC))
    ...
    def discovery_node_post_request(self):
        result = self.request_nothrow_discovery_node()
        if result is None:
            raise iconRPCError()
        return result
